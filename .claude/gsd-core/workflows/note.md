...
- **Global scope**: `D:/Repos/lancet/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Repos/lancet/.claude/notes/`)
...
2. Glob `D:/Repos/lancet/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Repos/lancet/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Repos/lancet/.claude/notes/` — works in any directory
